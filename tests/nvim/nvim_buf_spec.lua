...
describe('nvim_buf_attach() buf callbacks', function()
  it('should detach on buf unload', function()
    ...
    api.nvim_buf_attach(buf, false, {
      on_detach = function() detached = true end,
      on_lines = print,
    })
...
describe('nvim_buf_call()', function()
  before_each(function() vim.cmd 'silent %bw' end)
  ...
  it("shouldn't end insert mode", function()
    ...
    local mode_changes = util.autocmd('ModeChanged', { pattern = '*' }):await()
    ...
    for _ = 1, 2 do
      ...
      api.nvim_buf_call(buf1, function() vim.cmd 'silent write' end)

      api.nvim_buf_call(buf2, function() vim.cmd 'silent write' end)
    end
